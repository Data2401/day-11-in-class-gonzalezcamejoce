...
print(format(max_grant, scientific = FALSE))
```
...
```{r}
small_grant <- min(grants$total_amount, na.rm = TRUE)
print(small_grant)
```
...
```{r}
max_grant <- max(grants$total_amount, na.rm = TRUE)
largest_org <- grants$organization[grants$total_amount== max_grant]
print(largest_org)
...
```{r}
small_grant <- min(grants$total_amount, na.rm = TRUE)
smallest_org <- grants$organization[grants$total_amount== small_grant]
print(smallest_org)
```
...
```{r}
grants_awarded <- sum(grants$start_year == "2010", na.rm = TRUE)
print(grants_awarded)
```

- How much was awarded in 2010? 

```{r}
total_awarded <- sum(grants$total_amount[grants$start_year == "2010"], na.rm = TRUE)
print(total_awarded)
```
